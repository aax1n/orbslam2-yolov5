...
//      mvDynamicNames = {"person", "car", "motorbike", "bus", "bicycle" ,"train", "truck", "boat", "bird", "cat",
//                    "dog", "horse", "sheep", "crow", "bear", "book"}
...
bool YoloDetection::Detect()
{
    ...
    if (dets.size() > 0)
    {
        ...
        for (size_t i=0; i < dets[0].sizes()[0]; ++ i)
        {
            ...
            /// 静态物体 做语义用
            if (count(mvStaicNames.begin(), mvStaicNames.end(), mClassnames[classID]))
            {
                cv::Rect2i StaicnamicArea(left, top, (right - left), (bottom - top));  /// 矩形区域的四个点 left top
                mvDetectArea.push_back(StaicnamicArea);
            }
            cout <<GREEN<< "#--------------------------------------------#" << endl;
